...
contract Messaging {
    ...
    event HashMessage(address indexed _sender, address indexed _receiver, uint256 _time, bytes32 message);
    ...
    mapping (address => uint256) public last_hash_msg_index;
    mapping (address => mapping (uint256 => hash_message)) public hash_messages;
    ...
    function sendHashMessage(address _to, bytes32  _text)
    {
        hash_messages[_to][last_hash_msg_index[_to]].from = msg.sender;
        hash_messages[_to][last_hash_msg_index[_to]].text = _text;
        hash_messages[_to][last_hash_msg_index[_to]].time = now;
        last_hash_msg_index[_to]++;
        HashMessage(msg.sender, _to, now, _text);
    }
